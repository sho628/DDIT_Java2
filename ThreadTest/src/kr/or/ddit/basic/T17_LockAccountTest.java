...
/*
     ...
     ReentrantLock : Read 및 Write 구분없이 사용하기 위한 락 클래스로 동기화 처리를 위해
                     사용된다. (Synchronized를 이용한 동기화 처리보다 부가적인 기능을 제공함.)
                     
     ReentrantReadWriteLock : Read및 Write 락을 구분하여 사용 가능함.
*/
...
class LockAccount {
	...
	public void deposit(int money) {
		// Lock객체의  lock()메서드가 동기화 시작이고, unlock()메서드가 동기화 끝을 나타냄.
	...
	// 출금처리하는 메서드(출금 성공: true, 출금 실패: false 반환)
