...
public class T02_FileTest {
	...
	private static void displayFileList(File dir) {
		...
		// 하위 디렉토리정보를 저장할 ArrayList 생성(File배열의 인덱스 저장)
		...
		for(int i=0; i<files.length; i++) {
			String attr = ""; // 파일의 속성정보(읽기, 쓰기, 히든, 디렉토리여부)
			String size= "";	// 파일 용량
			
			if(files[i].isDirectory()) {
				attr = "<DIR>";
				subDirList.add(i);// 인덱스값을 List에 저장
			}else {
				size = files[i].length() + "";
		...
		int dirCount = subDirList.size(); // 폴더안의 하위 폴더 개수구하기
		...
		for(int i = 0; i<subDirList.size(); i++) {
			// 하위 폴더의 내용들도 출력하기 위해 현재 메서드를 재귀호출한다.
